...
void LIS2MDL_Magnetom_Init()
{
	...
	SPI_FD_Write(CFG_REG_A, CFG_REG_A_VAL); // Configure register REGA
	LIS2MDL_CS_High(); // Set Control Select pin low to finish a transaction

	LIS2MDL_CS_Low(); // Set Control Select pin high to start a transaction
	SPI_FD_Write(CFG_REG_B, CFG_REG_B_VAL); // Configure register REGB
	LIS2MDL_CS_High(); // Set Control Select pin low to finish a transaction

	LIS2MDL_CS_Low(); // Set Control Select pin high to start a transaction
	SPI_FD_Write(CFG_REG_C, CFG_REG_C_VAL); // Configure register REGC
	LIS2MDL_CS_High(); // Set Control Select pin low to finish a transaction
...
void LIS2MDL_Magnetom_Task()
{
	...
	Magnetic_Val[0] = SPI_FD_Read((uint8_t)OUTX_L_REG); // Read first byte of the 6-bytes representing the magnetic value
	Magnetic_Val[1] = SPI_FD_Read((uint8_t)OUTX_H_REG); // Read second byte of the 6-bytes representing the magnetic value
	Magnetic_Val[2] = SPI_FD_Read((uint8_t)OUTY_L_REG); // Read third byte of the 6-bytes representing the magnetic value
	Magnetic_Val[3] = SPI_FD_Read((uint8_t)OUTY_H_REG); // Read fourth byte of the 6-bytes representing the magnetic value
	Magnetic_Val[4] = SPI_FD_Read((uint8_t)OUTZ_L_REG); // Read fifth byte of the 6-bytes representing the magnetic value
	Magnetic_Val[5] = SPI_FD_Read((uint8_t)OUTZ_H_REG); // Read sixth byte of the 6-bytes representing the magnetic value
